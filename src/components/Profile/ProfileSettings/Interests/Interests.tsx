...
import { setInterests } from "@/Store/slices/profileSlice";
...
const Interests: FC = () => {
  ...
  const handleInterestClick = (interest: string) => {
    ...
    dispatch(setInterests(updatedInterests.length === 0 ? null : updatedInterests));
  };
      ...
      <div className={styles['interests']}>
        ...
        <div className={styles['interests-options']}>
          {interestsOptions.map((interest) => (
              <button
                  key={interest}
                  className={profile.interests && profile.interests.includes(interest) ? styles['selected'] : ''}
